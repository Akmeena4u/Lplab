...
int is_item_in_advanced(struct State* l,struct Rules r,int dot,int* 
bit)
{
int f = 0;
for(int i=0;i<l->len;i++)
...
{
if(is_item_in(&q->data,s->itm[i].r,s-
>itm[i].dotposition)==-1)
{
f = 1;break;
}
...
stack[top+1] = term[ff];
stack[top+2] = 
tab[stack[top]][ff].state_no;top+=2;ind++;
}
...
stack[top+1] = sto;
stack[top+2] = 
tab[stack[top]][find(sto)].state_no;top+=2;
}
}
// for(int i=0;i<n;i++)
// {
